"""Pipeline freshness and status helper for Myntra Discovery Engine."""
...
def get_pipeline_status() -> PipelineStatus:
    ...
    scores_path = ARTIFACT_DIR / OPPORTUNITY_SCORES_FILE
    meta_path = ARTIFACT_DIR / OPPORTUNITY_RUN_METADATA_FILE

    online = reviews_path.exists() and scores_path.exists()

    mtime = _file_mtime(scores_path) or _file_mtime(reviews_path)
    ...
    return PipelineStatus(
        ...
        opportunity_count=_count_json_array(scores_path),
        is_llm=is_llm,
        run_id=run_id,
    )
